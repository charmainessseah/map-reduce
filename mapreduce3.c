...
void Thread_Reduce(void* args) {
    ...
    for (int i = 0; i < word_count; i++) {
        if (num_unique_words != 0) {
            if (strcmp(unique_words[num_unique_words - 1], partition_list->elements[curr_partition_num]->list_of_words[word_count]) != 0) {
                unique_words[i] = strdup(partition_list->elements[curr_partition_num]->list_of_words[word_count]);
                printf("unique words for partition num %d: %s\n", curr_partition_num, unique_words[i]);
                num_unique_words++;
            }
        } else {
            unique_words[i] = strdup(partition_list->elements[curr_partition_num]->list_of_words[word_count]);
            ...
            Partitioner partition)
{
    ...
    for (int i = 0; i < num_partitions; i++) {
        ...
        for (int j = 0; j < partition_list->elements[i]->num_words; j++) {
            printf("word: %s, ", partition_list->elements[i]->list_of_words[j]);
        }
        printf("\n");
    }
